...
public class RetryTaskQueue {
    ...
    /**
     * Latest scheduled retry timestamp per deliveryId used to discard stale queue entries.
     */
    private final ConcurrentHashMap<String, Instant> schldByDeliveryId = new ConcurrentHashMap<>();
    ...
    public void schedule(String deliveryId, Instant nextAttemptAt) {
        ...
        Instant previous = schldByDeliveryId.put(deliveryId, nextAttemptAt);
    ...
    public RetryTask pollDue(Instant now) {
        while (true) {
            ...
            Instant currentSchedule = schldByDeliveryId.get(head.deliveryId());
            if (currentSchedule == null || !currentSchedule.equals(head.nextAttemptAt())) {
                continue;
            }
            schldByDeliveryId.remove(head.deliveryId(), currentSchedule);
            return head;
        }
    }
}
