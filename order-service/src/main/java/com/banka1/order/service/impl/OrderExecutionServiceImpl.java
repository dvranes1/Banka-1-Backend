...
public class OrderExecutionServiceImpl implements OrderExecutionService {
    ...
    public void executeOrderPortion(Order order) {
        ...
        ExecutionSettlement settlement = buildExecutionSettlement(managedOrder, listing, quantityToExecute, executionPricePerUnit);
        BigDecimal commission = listing.getListingType() == ListingType.FOREX
                ? BigDecimal.ZERO
                : calculateCommission(orderPricingFamily(managedOrder.getOrderType()),
                settlement.grossChunkAmount(), settlement.settlementCurrency());

        createTransaction(managedOrder, quantityToExecute, settlement.recordedPricePerUnit(), settlement.grossChunkAmount(), commission);
        ...
        finalizeActuaryExposure(managedOrder, settlement.settlementCurrency(), settlement.grossChunkAmount());
